...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] forgetful_own;
  vector<lower=0, upper=1>[N] forgetful_partner;
  vector[N] s_own;
  vector[N] s_partner;
  ...
  forgetful_own = inv_logit(mu_forgetful_own_nd + sigma_forgetful_own_nd * forgetful_own_nd);
  forgetful_partner = inv_logit(mu_forgetful_partner_nd + sigma_forgetful_partner_nd * forgetful_partner_nd);
  ...
  for (i in 1:N) { // Participant loop
    for (j in 1:S) { // Session loop
      ...
      for (t in 1:T) { // Trial loop
        ...
        Q_next[C[i, j, t]] += alpha_own[i] * (R[i, j, t] - Q[C[i, j, t]]);

        // Apply forgetting to other actions after own choice
        for (k in 1:NC) {
          if (k != C[i, j, t]) {
            Q_next[k] = forgetful_own[i] * initial_values[k] + (1 - forgetful_own[i]) * Q_next[k];
          }
        }

        // Update Q-values for partner's choice
        Q_next[PC[i, j, t]] += alpha_partner[i] * (PR[i, j, t] - Q_next[PC[i, j, t]]);

        // Apply forgetting to other actions after partner's choice
        for (k in 1:NC) {
          if (k != PC[i, j, t]) {
            Q_next[k] = forgetful_partner[i] * initial_values[k] + (1 - forgetful_partner[i]) * Q_next[k];
          }
        }
