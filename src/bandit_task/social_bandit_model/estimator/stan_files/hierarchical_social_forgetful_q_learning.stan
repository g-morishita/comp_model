...
transformed parameters {
  ...
  vector<lower=0,upper=1.0>[N] f; // transformed forgetfulness parameter [0, 1]

  // Transform the parameters
  alpha = inv_logit(mu_alpha_nd + exp(sigma_alpha_nd) * alpha_nd);
  beta = exp(mu_beta_nd + exp(sigma_beta_nd) * beta_nd);
  f = inv_logit(mu_f_nd + exp(sigma_f_nd) * f_nd);
}
  ...
  for ( i in 1:N ) { // participant
    for (j in 1:S) { // session
      ...
      for ( t in 1:T ) { // trial
        ...
        // Forget q values of unchosen actions
        for (k in 1:NC) {
            if (k != PC[i, j, t]) {
                Q[k] = f[i] * 0.5 + (1 - f[i]) * Q[k];
            }
        }
