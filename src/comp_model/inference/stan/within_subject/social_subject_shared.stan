// Stan program for social subject-shared Bayesian estimation.
...
transformed parameters {
  ...
  for (k in 1:K) {
    subject_log_scale[k] = -20.0;
  }
  for (b in 1:B) {
    for (k in 1:K) {
      block_z[b][k] = subject_param_z[k];
      if (transform_codes[k] == 0) {
        block_param[b][k] = subject_param_z[k];
      } else if (transform_codes[k] == 1) {
        block_param[b][k] = inv_logit(subject_param_z[k]);
      } else {
        block_param[b][k] = exp(subject_param_z[k]);
      }
    }
  }
}
...
model {
  for (k in 1:K) {
    target += normal_lpdf(subject_param_z[k] | mu_prior_mean[k], mu_prior_std[k]);
  }
...
generated quantities {
  ...
  for (k in 1:K) {
    log_prior_total += normal_lpdf(subject_param_z[k] | mu_prior_mean[k], mu_prior_std[k]);
  }
