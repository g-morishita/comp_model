...
transformed parameters {
  ...
  for (k in 1:K) {
    ...
    if (transform_codes[k] == 0) {
      subject_param[k] = subject_param_z[k];
    } else if (transform_codes[k] == 1) {
      subject_param[k] = inv_logit(subject_param_z[k]);
    } else {
      subject_param[k] = exp(subject_param_z[k]);
    }
  }
  ...
  for (b in 1:B) {
    for (k in 1:K) {
      ...
      block_param[b][k] = subject_param[k];
    }
  }
}
