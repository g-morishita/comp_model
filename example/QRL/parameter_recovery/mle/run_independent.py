...
def main():
    param_config = load_parameter_recovery_config("param_recovery_configs/independent.yaml")
    results = run_parameter_recovery(config=param_config)
    plot_paths = plot_parameter_recovery(outputs=results, out_dir=f"{results.out_dir}/plots", split_by_rep=True)
    if plot_paths:
