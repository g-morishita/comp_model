...
parameters {
  real alpha_o_shared_z;
  vector[C-1] alpha_o_delta_z;
...
transformed parameters {
  ...
  for (c in 1:C) {
    if (c == baseline_cond) {
      alpha_o_z[c] = alpha_o_shared_z;
      beta_z[c] = beta_shared_z;
    } else {
      int idx = (c < baseline_cond) ? c : (c - 1);
      alpha_o_z[c] = alpha_o_shared_z + alpha_o_delta_z[idx];
...
model {
  target += prior_lpdf(alpha_o_shared_z | alpha_o_shared_prior_family, alpha_o_shared_prior_p1, alpha_o_shared_prior_p2, alpha_o_shared_prior_p3);
  target += prior_lpdf(beta_shared_z | beta_shared_prior_family, beta_shared_prior_p1, beta_shared_prior_p2, beta_shared_prior_p3);

  for (i in 1:(C-1)) {
    target += prior_lpdf(alpha_o_delta_z[i] | alpha_o_delta_prior_family, alpha_o_delta_prior_p1, alpha_o_delta_prior_p2, alpha_o_delta_prior_p3);
...
generated quantities {
  ...
  real alpha_o_shared_z_hat = alpha_o_shared_z;
  real beta_shared_z_hat = beta_shared_z;

  vector[C-1] alpha_o_delta_z_hat = alpha_o_delta_z;
