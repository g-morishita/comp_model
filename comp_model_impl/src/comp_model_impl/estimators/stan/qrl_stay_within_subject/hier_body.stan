data {
  ...
  int<lower=1, upper=8> mu_alpha__shared_prior_family; real mu_alpha__shared_prior_p1; real mu_alpha__shared_prior_p2; real mu_alpha__shared_prior_p3;
  int<lower=1, upper=8> sd_alpha__shared_prior_family; real sd_alpha__shared_prior_p1; real sd_alpha__shared_prior_p2; real sd_alpha__shared_prior_p3;
  ...
  int<lower=1, upper=8> mu_alpha__delta_prior_family; real mu_alpha__delta_prior_p1; real mu_alpha__delta_prior_p2; real mu_alpha__delta_prior_p3;
  int<lower=1, upper=8> sd_alpha__delta_prior_family; real sd_alpha__delta_prior_p1; real sd_alpha__delta_prior_p2; real sd_alpha__delta_prior_p3;
...
parameters {
  // shared (non-centered)
  real mu_alpha__shared; real<lower=0> sd_alpha__shared; vector[N] z_alpha__shared;
  ...
  vector[C-1] mu_alpha__delta; vector<lower=0>[C-1] sd_alpha__delta; matrix[N, C-1] z_alpha__delta;
...
transformed parameters {
  vector[N] alpha__shared_z = mu_alpha__shared + sd_alpha__shared * z_alpha__shared;
  ...
  for (n in 1:N) {
    for (i in 1:(C - 1)) {
      alpha__delta_z[n, i] = mu_alpha__delta[i] + sd_alpha__delta[i] * z_alpha__delta[n, i];
  ...
  for (n in 1:N) {
    for (c in 1:C) {
      if (c == baseline_cond) {
        alpha_z[n, c] = alpha__shared_z[n];
        beta_z[n, c] = beta__shared_z[n];
        kappa[n, c] = kappa__shared_z[n];
      } else {
        int idx = (c < baseline_cond) ? c : (c - 1);
        alpha_z[n, c] = alpha__shared_z[n] + alpha__delta_z[n, idx];
        beta_z[n, c] = beta__shared_z[n] + beta__delta_z[n, idx];
        kappa[n, c] = kappa__shared_z[n] + kappa__delta_z[n, idx];
      }
    }
  }

  matrix<lower=0, upper=1>[N, C] alpha = inv_logit(alpha_z);
  ...
  for (c in 1:C) {
    if (c == baseline_cond) {
      alpha_pop[c] = inv_logit(mu_alpha__shared);
      beta_pop[c] = beta_lower + exp(mu_beta__shared);
      kappa_pop[c] = mu_kappa__shared;
    } else {
      int idx = (c < baseline_cond) ? c : (c - 1);
      alpha_pop[c] = inv_logit(mu_alpha__shared + mu_alpha__delta[idx]);
      beta_pop[c] = beta_lower + exp(mu_beta__shared + mu_beta__delta[idx]);
      kappa_pop[c] = mu_kappa__shared + mu_kappa__delta[idx];
    }
  }
}
model {
  z_alpha__shared ~ normal(0, 1);
  z_beta__shared ~ normal(0, 1);
  z_kappa__shared ~ normal(0, 1);

  to_vector(z_alpha__delta) ~ normal(0, 1);
  to_vector(z_beta__delta) ~ normal(0, 1);
  to_vector(z_kappa__delta) ~ normal(0, 1);

  // shared hyperpriors
  target += prior_lpdf(mu_alpha__shared | mu_alpha__shared_prior_family, mu_alpha__shared_prior_p1, mu_alpha__shared_prior_p2, mu_alpha__shared_prior_p3);
  target += prior_lpdf(sd_alpha__shared | sd_alpha__shared_prior_family, sd_alpha__shared_prior_p1, sd_alpha__shared_prior_p2, sd_alpha__shared_prior_p3);
  ...
  // delta hyperpriors
  for (cc in 1:(C - 1)) {
    target += prior_lpdf(mu_alpha__delta[cc] | mu_alpha__delta_prior_family, mu_alpha__delta_prior_p1, mu_alpha__delta_prior_p2, mu_alpha__delta_prior_p3);
    target += prior_lpdf(sd_alpha__delta[cc] | sd_alpha__delta_prior_family, sd_alpha__delta_prior_p1, sd_alpha__delta_prior_p2, sd_alpha__delta_prior_p3);
  ...
  for (e in 1:E) {
    ...
    if (etype[e] == 1) {
      ...
      if (action[e] > 0) {
        ...
        Q[n][s, a] = Q[n][s, a] + alpha[n, c] * (r - Q[n][s, a]);
        last_choice[n, s] = a;
      }
    }
  }
...
generated quantities {
  ...
  vector[C - 1] mu_alpha__delta_hat = mu_alpha__delta;
  vector[C - 1] sd_alpha__delta_hat = sd_alpha__delta;
