"""Stan adapter for the Vicarious AP-DB-Stay model.

This adapter maps
:class:`comp_model_impl.models.vicarious_ap_db_stay.vicarious_ap_db_stay.Vicarious_AP_DB_STAY`
...
The Stan templates for this adapter are located at::
    ...
    comp_model_impl/estimators/stan/vicarious_ap_db_stay/indiv_body.stan
    comp_model_impl/estimators/stan/vicarious_ap_db_stay/hier_body.stan
"""
...
from comp_model_impl.models.vicarious_ap_db_stay.vicarious_ap_db_stay import Vicarious_AP_DB_STAY

from .base import StanAdapter, StanProgramRef


@dataclass(frozen=True, slots=True)
class VicariousAPDBStayStanAdapter(StanAdapter):
    """Adapter that maps :class:`Vicarious_AP_DB_STAY` to Stan templates.

    Parameters
    ----------
    model : comp_model_core.interfaces.model.ComputationalModel
        Must be an instance of :class:`Vicarious_AP_DB_STAY`.

    Examples
    --------
    >>> from comp_model_impl.models import Vicarious_AP_DB_STAY
    >>> adapter = VicariousAPDBStayStanAdapter(model=Vicarious_AP_DB_STAY())
    ...
    def __post_init__(self) -> None:
        """Validate that the wrapped model is a Vicarious_AP_DB_STAY instance.

        Raises
        ------
        TypeError
            If ``model`` is not a :class:`Vicarious_AP_DB_STAY`.
        """
        if not isinstance(self.model, Vicarious_AP_DB_STAY):
            raise TypeError(
                f"{self.__class__.__name__} requires {Vicarious_AP_DB_STAY.__name__}, "
    ...
    def program(self, family: str) -> StanProgramRef:
        ...
        """
        return StanProgramRef(family=family, key="vicarious_ap_db_stay", program_name=f"vicarious_ap_db_stay_{family}")
    ...
    def required_priors(self, family: str) -> Sequence[str]:
        ...
        """
        if family == "indiv":
            return ["alpha_o", "alpha_a", "demo_bias_rel", "beta", "kappa"]
        if family == "hier":
            return [
                "mu_alpha_o",
                "sd_alpha_o",
