"""Stan adapter for within-subject (shared+delta) VicQ_AP_DualW_Stay models."""
...
from ....models import VicQ_AP_DualW_Stay
...
class VicQAPDualWStayWithinSubjectStanAdapter(StanAdapter):
    """Adapter for :class:`VicQ_AP_DualW_Stay` wrapped in shared+delta conditioner."""
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(base, VicQ_AP_DualW_Stay):
            raise TypeError(
                f"{self.__class__.__name__} requires base_model={VicQ_AP_DualW_Stay.__name__}, "
                f"got {type(base).__name__}"
            )

    @property
    def base_model(self) -> VicQ_AP_DualW_Stay:
        return getattr(self.model, "base_model")

    def program(self, family: str) -> StanProgramRef:
        return StanProgramRef(
            family=family,
            key="vicQ_ap_dualw_stay_within_subject",
            program_name=f"vicQ_ap_dualw_stay_within_subject_{family}",
        )
