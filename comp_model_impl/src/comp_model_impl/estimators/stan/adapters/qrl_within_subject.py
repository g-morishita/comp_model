"""Stan adapter for within-subject (shared+delta) QRL models."""
...
class QRLWithinSubjectStanAdapter(StanAdapter):
    """Adapter for :class:`QRL` wrapped in shared+delta conditioner."""
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(base, QRL):
            raise TypeError(
                f"{self.__class__.__name__} requires base_model={QRL.__name__}, "
                f"got {type(base).__name__}"
            )

    @property
    def base_model(self) -> QRL:
        return getattr(self.model, "base_model")

    def program(self, family: str) -> StanProgramRef:
        return StanProgramRef(
            family=family,
            key="qrl_within_subject",
            program_name=f"qrl_within_subject_{family}",
        )
