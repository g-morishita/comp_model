"""Stan adapter for the Vicarious Dirichlet-DB-Stay model.

This adapter maps
:class:`comp_model_impl.models.vicarious_dir_db_stay.vicarious_dir_db_stay.Vicarious_Dir_DB_Stay`
...
The Stan templates for this adapter are located at::
    ...
    comp_model_impl/estimators/stan/vicarious_dir_db_stay/indiv_body.stan
    comp_model_impl/estimators/stan/vicarious_dir_db_stay/hier_body.stan
"""
...
from comp_model_impl.models.vicarious_dir_db_stay.vicarious_dir_db_stay import Vicarious_Dir_DB_Stay

from .base import StanAdapter, StanProgramRef


@dataclass(frozen=True, slots=True)
class VicariousDirDBStayStanAdapter(StanAdapter):
    """Adapter that maps :class:`Vicarious_Dir_DB_Stay` to Stan templates.

    Parameters
    ----------
    model : comp_model_core.interfaces.model.ComputationalModel
        Must be an instance of :class:`Vicarious_Dir_DB_Stay`.

    Examples
    --------
    >>> from comp_model_impl.models import Vicarious_Dir_DB_Stay
    >>> adapter = VicariousDirDBStayStanAdapter(model=Vicarious_Dir_DB_Stay())
    ...
    def __post_init__(self) -> None:
        """Validate that the wrapped model is a Vicarious_Dir_DB_Stay instance.

        Raises
        ------
        TypeError
            If ``model`` is not a :class:`Vicarious_Dir_DB_Stay`.
        """
        if not isinstance(self.model, Vicarious_Dir_DB_Stay):
            raise TypeError(
                f"{self.__class__.__name__} requires {Vicarious_Dir_DB_Stay.__name__}, "
    ...
    def program(self, family: str) -> StanProgramRef:
        ...
        """
        return StanProgramRef(family=family, key="vicarious_dir_db_stay", program_name=f"vicarious_dir_db_stay_{family}")
    ...
    def required_priors(self, family: str) -> Sequence[str]:
        ...
        """
        if family == "indiv":
            return ["alpha_o", "demo_bias_rel", "beta", "kappa"]
        if family == "hier":
            return [
                "mu_alpha_o",
                "sd_alpha_o",
    ...
    def augment_subject_data(self, data: dict[str, Any]) -> None:
        ...
        data["demo_dirichlet_prior"] = float(getattr(self.model, "demo_dirichlet_prior", 1.0))
    ...
    def population_var_names(self) -> Sequence[str]:
        ...
        return [
            ...
            "mu_alpha_o_hat",
            "sd_alpha_o_hat",
            "mu_demo_bias_rel_hat",
            "sd_demo_bias_rel_hat",
            "mu_beta_hat",
            "sd_beta_hat",
            "mu_kappa_hat",
            "sd_kappa_hat",
