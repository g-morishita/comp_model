"""Stan adapter for within-subject (shared+delta) AP_RL_Stay models."""
...
class APRLStayWithinSubjectStanAdapter(StanAdapter):
    """Adapter for :class:`AP_RL_Stay` wrapped in shared+delta conditioner."""
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(base, AP_RL_Stay):
            raise TypeError(
                f"{self.__class__.__name__} requires base_model={AP_RL_Stay.__name__}, "
                f"got {type(base).__name__}"
            )

    @property
    def base_model(self) -> AP_RL_Stay:
        return getattr(self.model, "base_model")

    def program(self, family: str) -> StanProgramRef:
        return StanProgramRef(
            family=family,
            key="ap_rl_stay_within_subject",
            program_name=f"ap_rl_stay_within_subject_{family}",
        )
