...
parameters {
  real alpha_p_shared_z;
  vector[C-1] alpha_p_delta_z;

  real alpha_i_shared_z;
  vector[C-1] alpha_i_delta_z;

  real beta_shared_z;
  vector[C-1] beta_delta_z;

  real kappa_shared_z;
  vector[C-1] kappa_delta_z;
}
transformed parameters {
  ...
  for (c in 1:C) {
    if (c == baseline_cond) {
      alpha_p_z[c] = alpha_p_shared_z;
      alpha_i_z[c] = alpha_i_shared_z;
      beta_z[c] = beta_shared_z;
      kappa_z[c] = kappa_shared_z;
    } else {
      int idx = (c < baseline_cond) ? c : (c - 1);
      alpha_p_z[c] = alpha_p_shared_z + alpha_p_delta_z[idx];
      alpha_i_z[c] = alpha_i_shared_z + alpha_i_delta_z[idx];
      beta_z[c] = beta_shared_z + beta_delta_z[idx];
      kappa_z[c] = kappa_shared_z + kappa_delta_z[idx];
    }
  }
...
model {
  target += prior_lpdf(alpha_p_shared_z, alpha_p_shared_prior_family, alpha_p_shared_prior_p1, alpha_p_shared_prior_p2, alpha_p_shared_prior_p3);
  target += prior_lpdf(alpha_i_shared_z, alpha_i_shared_prior_family, alpha_i_shared_prior_p1, alpha_i_shared_prior_p2, alpha_i_shared_prior_p3);
  target += prior_lpdf(beta_shared_z,    beta_shared_prior_family,    beta_shared_prior_p1,    beta_shared_prior_p2,    beta_shared_prior_p3);
  target += prior_lpdf(kappa_shared_z,   kappa_shared_prior_family,   kappa_shared_prior_p1,   kappa_shared_prior_p2,   kappa_shared_prior_p3);

  for (i in 1:(C-1)) {
    target += prior_lpdf(alpha_p_delta_z[i], alpha_p_delta_prior_family, alpha_p_delta_prior_p1, alpha_p_delta_prior_p2, alpha_p_delta_prior_p3);
    target += prior_lpdf(alpha_i_delta_z[i], alpha_i_delta_prior_family, alpha_i_delta_prior_p1, alpha_i_delta_prior_p2, alpha_i_delta_prior_p3);
    target += prior_lpdf(beta_delta_z[i],    beta_delta_prior_family,    beta_delta_prior_p1,    beta_delta_prior_p2,    beta_delta_prior_p3);
    target += prior_lpdf(kappa_delta_z[i],   kappa_delta_prior_family,   kappa_delta_prior_p1,   kappa_delta_prior_p2,   kappa_delta_prior_p3);
  }
...
generated quantities {
  ...
  real alpha_p_shared_z_hat = alpha_p_shared_z;
  real alpha_i_shared_z_hat = alpha_i_shared_z;
  real beta_shared_z_hat = beta_shared_z;
  real kappa_shared_z_hat = kappa_shared_z;

  vector[C-1] alpha_p_delta_z_hat = alpha_p_delta_z;
  vector[C-1] alpha_i_delta_z_hat = alpha_i_delta_z;
  vector[C-1] beta_delta_z_hat = beta_delta_z;
  vector[C-1] kappa_delta_z_hat = kappa_delta_z;
}
