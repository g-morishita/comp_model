...
class ModelRecoveryConfig:
    ...
    seed : int, default=0
        Base random seed.
    n_fit_jobs : int, default=1
        Number of worker processes used to parallelize model fittings.
    ...
    def __post_init__(self) -> None:
        ...
        if int(self.n_fit_jobs) < 1:
            raise ValueError("n_fit_jobs must be >= 1")
...
def config_from_raw_dict(raw: Mapping[str, Any]) -> ModelRecoveryConfig:
    ...
    return ModelRecoveryConfig(
        ...
        n_fit_jobs=int(raw.get("n_fit_jobs", 1)),
