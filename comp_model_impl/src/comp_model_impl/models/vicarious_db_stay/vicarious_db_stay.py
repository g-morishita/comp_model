"""Vicarious DB model with perseveration (stay), without reliability.

This model matches Vicarious Dirichlet-DB-Stay except the demonstrator policy
is not tracked and no reliability scaling is applied to the demo-choice bias.
"""
...
class Vicarious_DB_Stay(SocialComputationalModel):
    """Vicarious DB model with perseveration (stay), without reliability.
    ...
    demo_bias : float, optional
        Demonstrator-choice decision bias (constant; no reliability scaling)
    beta : float, optional
        Inverse temperature.
    ...
    --------------
        ...
        The agent biases toward (or away from) the demonstrator's action during
        decision making, without estimating demonstrator reliability.
        ...
        • demonstrator-choice bias (constant)

        P(a_t = i) ∝ exp(
            beta * Q_t(i)
            + kappa * I[i == a_{t-1}]
            + demo_bias * I[i == demo_a]
        )

    Notes
    -----
    • Action values are updated only from demonstrator outcomes.
    • Reliability is not estimated; demo-choice bias is constant.
    """
    alpha_o: float = 0.2
    demo_bias: float = 1.0
    beta: float = 3.0
    ...
    def param_schema(self) -> ParameterSchema:
        """Return the parameter schema for Vicarious DB-Stay model.
        ...
        return vicarious_db_stay_schema(
            ...
            demo_bias_default=float(self.demo_bias),
            beta_default=float(self.beta),
            kappa_default=float(self.kappa),
            demo_bias_abs_max=float(self.demo_bias_abs_max),
    ...
    def action_probs(self, *, state: Any, spec: EnvironmentSpec) -> np.ndarray:
        ...
        - demonstrator-choice bias: demo_bias * 1[a == recent_demo_choice]
