"""Vicarious Dirichlet-DB model with perseveration (stay).

This model matches Vicarious AP-DB-Stay except the demonstrator policy is
tracked via a Bayesian Dirichlet update (no alpha_a parameter).
"""
...
from ..vicarious_ap_db_stay.vicarious_ap_db_stay import determinism_index_reliability


@dataclass(slots=True)
class Vicarious_Dir_DB_Stay(SocialComputationalModel):
    """Vicarious Dirichlet-DB model with perseveration (stay).
    ...
    --------------
        ...
        The agent additionally infers the demonstrator’s action policy via a
        Dirichlet posterior and computes partner reliability. Reliability
        modulates copying of the demonstrator’s action during decision making.
        ...
        1) Demonstrator action-policy learning (Dirichlet update)
        The agent maintains action-counts ``c`` over K actions. With a symmetric
        Dirichlet prior, the posterior mean is:

            demo_pi_t(i) = c_t(i) / sum_j c_t(j)

        After observing demonstrator action demo_a at trial t:

            c_{t+1}(demo_a) = c_t(demo_a) + 1
        ...
        Reliability is computed using a normalized maximum-probability index:
            ...
            Rel_t = (max_i demo_pi_t(i) - 1/K) / (1 - 1/K)
    ...
    _demo_counts: Sequence[float] = field(default_factory=list)
    ...
    def param_schema(self) -> ParameterSchema:
        """Return the parameter schema for Vicarious Dirichlet-DB-Stay model.
        ...
        """
        return vicarious_dir_db_stay_schema(
    ...
    def reset_block(self, *, spec: EnvironmentSpec) -> None:
        ...
        self._q = np.tile(float(self._init_q_value), nA)
        self._demo_counts = np.full(nA, float(self.demo_dirichlet_prior), dtype=float)
        self._last_self_choice = None
        self._recent_demo_choice = None
    ...
    def action_probs(self, *, state: Any, spec: EnvironmentSpec) -> np.ndarray:
        ...
        demo_counts = np.asarray(self._demo_counts, dtype=float)
        s = demo_counts.sum()
        demo_pi = demo_counts / s if s > 0 else np.ones(nA, dtype=float) / nA
    ...
    def social_update(
        ...
        *,
        ...
        if 0 <= d < nA:
            self._demo_counts = np.asarray(self._demo_counts, dtype=float)
            self._demo_counts[d] = self._demo_counts[d] + 1.0
