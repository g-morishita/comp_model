# comp_model Documentation

`comp_model` provides a generic framework for computational decision modeling:
simulation, replay, inference, model comparison, and recovery.

GitHub repository: <https://github.com/g-morishita/comp_model>.

## Start here

- Tutorials overview: [Tutorials](tutorials/index.md)
- New user: start with [Install and Verify](tutorials/install-and-verify.md)
- Core workflow: [First End-to-End Fit](tutorials/first-end-to-end-fit.md)
- Recovery tutorials: [Parameter Recovery](tutorials/parameter-recovery.md) and
  [Model Recovery](tutorials/model-recovery.md)
- Use your own data last: [Fit Your Own Dataset](tutorials/fit-your-own-model-dataset-RL.md)
- Advanced workflow: [End-to-End Hierarchical Bayesian Fit](tutorials/end-to-end-hierarchical-bayesian-fit.md)

## How-to guides

- [Create a New Problem](how-to/create-a-new-problem.md)
- [Build a Multi-Phase Social Problem](how-to/build-a-multi-phase-social-problem.md)
- [Connect a Problem, Generator, and Demonstrator](how-to/connect-problem-generator-and-demonstrator.md)
- [Run Recovery Workflows](how-to/run-recovery-workflows.md)
- [Compare Candidate Models](how-to/compare-candidate-models.md)
- [Choose a Block Fit Strategy](how-to/choose-block-fit-strategy.md)
- [Fit Existing CSV Data](how-to/fit-existing-csv-data.md)
- [Fit Multiple Blocks with One Parameter Set](how-to/fit-multiple-blocks-with-one-parameter-set.md)
- [Fit Bayesian Hierarchical Models](how-to/how-to-fit-bayesian-hierarchical-model.md)
- [Deploy Docs and Enable Hooks](how-to/deploy-docs-and-enable-hooks.md)
- [Contribute New Models and Tasks](how-to/contribute-new-models-and-tasks.md)

## Reference

- [API Overview](reference/api-overview.md)
- [Configuration Schemas](reference/config_schemas.md)
- [Contribution Rules](reference/contribution-rules.md)

## Explanation

- [Design Philosophy](explanation/design-philosophy.md)
- [Architecture Decisions](explanation/architecture-decisions.md)

## Contribution

- [Contribution Rules](reference/contribution-rules.md): repository-level rules and conventions.
- [Contribute New Models and Tasks](how-to/contribute-new-models-and-tasks.md): step-by-step checklist.
